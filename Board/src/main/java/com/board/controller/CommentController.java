...
public class CommentController {
	...
	public JsonObject registerComment(@PathVariable(value = "idx", required = false) Long idx, @RequestBody final CommentDTO params) {
		...
		try {
			...
			jsonObj.addProperty("message", "데이터베이스 처리 과정에 문제가 발생함.");
		} catch(Exception e) {
			jsonObj.addProperty("message", "시스템에 문제가 발생함.");
		}
	...
	public JsonObject deleteComment(@PathVariable("idx") final Long idx) {
		...
		try {
			...
			jsonObj.addProperty("message", "데이터베이스 처리 과정에 문제가 생김.");
		} catch(Exception e) {
			jsonObj.addProperty("message", "시스템에 문제가 발생했음.");
		}
